...
export default function ProposalDetail({ proposal: initial }: { proposal: Proposal }) {
  ...
  const handleSend = async () => {
    if (!proposal.customer_email) {
      setError('No email address on file for this customer')
      return
    }
    setSending(true)
    setError(null)
        ...
        {proposal.status === 'signed' ? (
          ...
          <div className="flex gap-2">
            <button
              ...
              style={{
                background: sending ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.08)',
                color: sending ? '#6B7280' : '#D1D5DB',
                border: '1px solid rgba(255,255,255,0.10)',
              }}>
              {sending ? (
                <><div className="w-4 h-4 border-2 border-white/20 border-t-white/60 rounded-full animate-spin" />Sending…</>
              ) : (
                <><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  <line x1="22" y1="2" x2="11" y2="13" /><polygon points="22 2 15 22 11 13 2 9 22 2" />
                </svg>{sendDone ? 'Resend' : 'Send'}</>
              )}
            </button>
            <button
              type="button"
