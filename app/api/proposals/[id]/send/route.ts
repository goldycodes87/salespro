...
export async function POST(
  ...
  const publicUrl = `${process.env.NEXT_PUBLIC_SITE_URL ?? 'https://salespro-lake.vercel.app'}/p/${proposal.public_token}`

  const pricing = proposal.pricing_data ?? {}
  const financing = pricing.financing && pricing.financing !== 'none'
  ...
  if (proposal.lead_id) {
    try {
      await admin.from('lead_activity').insert({
        ...
        description: `Proposal sent to ${toEmail}`,
      })
    } catch {}
  }
